...
export const UploadProvider = ({ children }) => {
    ...
    pollingIntervalRef.current = setInterval(async () => {
      try {
        ...
        for (const upload of processingUploads) {
          ...
          const res = await fetch(`https://osimap-web-system.onrender.com/status?taskId=${upload.taskId}`);
          if (!res.ok) continue;
  ...
  const cancelUpload = async () => {
    try {
      const res = await fetch('https://osimap-web-system.onrender.com/cancel', { method: 'POST' });
